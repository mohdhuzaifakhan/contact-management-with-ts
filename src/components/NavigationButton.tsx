...
const NavigationButton: React.FC<NavigationButtonProps> = ({ open, setOpen }) => {
    return (
        <div className="">
            {!open ? (
                <div className="flex w-full">
                    <a onClick={() => setOpen(!open)} className={`w-full flex flex-row justify-center items-center h-10`}>
                        <span className="flex text-center rounded-full items-center hover:bg-gray-50 p-2 text-gray-600 hover:text-gray-800">
                            <svg className="h-5 w-5" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#716a6a">
